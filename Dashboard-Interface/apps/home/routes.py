...
# attendance_df.insert(0, "No", range(1, len(attendance_df) + 1))
session_count = 1
...
@login_required
def process_frame():
    global attendance_df, session_count, detected_faces_set, detection_dates, detection_start_times
    ...
    img = img.convert('RGB')
    img_np = np.array(img)
    resized_frame = cv2.resize(img_np, (640, 480)) #Resize img 
    ...
    # Create new session count 
    session_col = f"Session {session_count} {today_str}"
    if session_col not in attendance_df.columns:
        attendance_df[session_col] = 0  # Initialize new session column with 0 (not detected)
        session_count += 1  # Increment session count after adding the column

    for result in results:
        for box, conf, cls in zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
            if conf > 0.9:  # Only consider faces with high confidence
                ...
                # If the person has been detected for more than the minimum duration, log it
                if detection_start_times.get(name):
                    duration = current_time - detection_start_times[name]
                    if duration >= 2:  # Minimum duration of detection (1 second in this case)
                        ...
                        # Mark attendance in the session column (1 for detected)
